...
def loglike_sr_2param_minus(cube, ndim, nparams):
    ...
    eves_gen_nuebar.simulate_eves_rectangle_rule(n_samples=100, gL_mod=gL_mod, gR_mod=gR_mod)
    eves_gen_numu.simulate_eves_rectangle_rule(n_samples=100, gL_mod=gL_mod, gR_mod=gR_mod)
...
def loglike_sr_2param_plus(cube, ndim, nparams):
    ...
    # Generate events based on parameters (assuming predefined fluxes and material)
    eves_gen_nue.simulate_eves_rectangle_rule(n_samples=100, gL_mod=gL_mod, gR_mod=gR_mod)
    eves_gen_numubar.simulate_eves_rectangle_rule(n_samples=100, gL_mod=gL_mod, gR_mod=gR_mod)
...
def run_multinest_2param(muon_com_energy, scenario, resume=False):
    ...
    if scenario == "MC":
        ...
        pymultinest.run(loglike_sr_2param_minus, prior_2param, 2, outputfiles_basename=f'multinest/{scenario}/{muon_com_energy}_{scenario}_2param_',
                        n_live_points=500, resume=resume, verbose=True)
    elif scenario == "SR_PLUS":
        pymultinest.run(loglike_sr_2param_plus, prior_2param, 2, outputfiles_basename=f'multinest/{scenario}/{muon_com_energy}_{scenario}_2param_',
                        n_live_points=500, resume=resume, verbose=True)
...
if __name__ == "__main__":
    ...
    run_multinest_2param("sqrts-10TeV", "SR", resume=False)
