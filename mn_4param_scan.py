...
def loglike_sr_minus(cube, ndim, nparams):
    g_nue = cube[0]
    g_numu = cube[1]
    g_L = cube[2]
    g_R = cube[3]
...
def loglike_sr_plus(cube, ndim, nparams):
    g_nue = cube[0]
    g_numu = cube[1]
    g_L = cube[2]
    g_R = cube[3]
...
def prior(cube, ndim, nparams):
    # Define flat priors
    cube[0] = 2*cube[0] - 1  # g_nue, [-1, 1]
    cube[1] = 2*cube[1] - 1  # g_numu, [-1, 1]
    cube[2] = 2*cube[2] - 1  # g_L, [-1, 1]
    cube[3] = 2*cube[3] - 1  # g_R, [-1, 1]
...
if __name__ == "__main__":
    ...
    run_multinest("sqrts-10TeV", "MC", resume=False)
