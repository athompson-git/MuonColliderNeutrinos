...
# charge radius definitions
cr_e_SM = HBARC**2 * G_F * (3 - 2 * np.log(np.power(M_E/M_W, 2))) / (4*sqrt(2)*pi**2)
cr_mu_SM = HBARC**2 * G_F * (3 - 2 * np.log(np.power(M_MU/M_W, 2))) / (4*sqrt(2)*pi**2)
...
eves_gen_nuebar.simulate_eves_rectangle_rule(n_samples=100, cr=cr_e_SM)
eves_gen_numu.simulate_eves_rectangle_rule(n_samples=100, cr=cr_mu_SM)
eves_gen_nue.simulate_eves_rectangle_rule(n_samples=100, cr=cr_e_SM)
eves_gen_numubar.simulate_eves_rectangle_rule(n_samples=100, cr=cr_mu_SM)
...
def loglike_sr_2param_minus(cube, ndim, nparams):
    cr_nue = cube[0] * 1e-32
    cr_numu = cube[1] * 1e-32

    # Generate events based on parameters (assuming predefined fluxes and material)
    eves_gen_nuebar.simulate_eves_rectangle_rule(n_samples=100, cr=cr_nue)
    eves_gen_numu.simulate_eves_rectangle_rule(n_samples=100, cr=cr_numu)
...
def loglike_sr_2param_plus(cube, ndim, nparams):
    ...
    # Generate events based on parameters (assuming predefined fluxes and material)
    eves_gen_nue.simulate_eves_rectangle_rule(n_samples=100, cr=cr_nue)
    eves_gen_numubar.simulate_eves_rectangle_rule(n_samples=100, cr=cr_numu)
...
def run_multinest_2param(muon_com_energy, scenario, resume=False):
    ...
    if scenario == "MC":
        pymultinest.run(loglike_mc_2param, prior_2param, 2, outputfiles_basename=f'multinest/{scenario}/{muon_com_energy}_{scenario}_cr_',
                        n_live_points=500, resume=resume, verbose=True)
    elif scenario == "SR_MINUS":
        pymultinest.run(loglike_sr_2param_minus, prior_2param, 2, outputfiles_basename=f'multinest/{scenario}/{muon_com_energy}_{scenario}_cr_',
                        n_live_points=500, resume=resume, verbose=True)
    elif scenario == "SR_PLUS":
        pymultinest.run(loglike_sr_2param_plus, prior_2param, 2, outputfiles_basename=f'multinest/{scenario}/{muon_com_energy}_{scenario}_cr_',
                        n_live_points=500, resume=resume, verbose=True)
